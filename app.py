"""
DeepSeek Balance — Windows desktop widget.

A frameless glass card that shows what is left in the DeepSeek wallet, and how
much the last stretch of API use cost, derived from consecutive balance readings.
...
icon and the low-balance notification are pystray + shell balloons.

All wallet logic lives in balance.py — this file only hosts the window, keeps
preferences, and performs the one network call when it is asked to.
"""
...
SHORTCUT_NAME = "DeepSeek Balance Widget.lnk"
WINDOW_TITLE = "DeepSeek Balance"

DEFAULT_WINDOW = {"width": 200, "height": 100, "x": None, "y": None}
WINDOW_MARGIN = 18
...
MUTEX_NAME = "47Lab.DeepSeekBalance.Widget"
...
class Widget:
    def __init__(self) -> None:
        ...
        # Readings live on disk so spend survives a restart, and the last one is shown
        # straight away (with its age) instead of an empty card.
        self.store = engine.Store(ROOT / "history.json")
        self.reading: engine.Reading | None = self.store.last()
        self.error: engine.BalanceError | None = None
    ...
    def wallet_state(self) -> str:
        """One word for the card and the tray dot."""
        if self.reading is None:
            return self.error.kind if self.error else "no_key"
        if not self.reading.is_available:
            return "low"
    ...
    def payload(self) -> dict:
        """What the card renders. Cached state only -- this must never touch the
        network, because the tray, the tooltip and every preference change call it."""
        reading = self.reading
        state = self.wallet_state()
        currency = reading.currency if reading else "USD"
        problem = str(self.error) if self.error else ""
        body = {
            "state": state,
            "problem": problem,
            "prefs": self.prefs(),
            ...
            "key_file": str(engine.DEFAULT_KEY_FILE),
            "key_present": engine.read_key() is not None,
            "reads": len(self.store.rows),
            "currency": currency,
            "amount": engine.money(reading.total, currency) if reading else "--",
            "balance": reading.total if reading else None,
            "granted": engine.money(reading.granted, currency) if reading else "",
            "topped_up": engine.money(reading.topped_up, currency) if reading else "",
            "available": reading.is_available if reading else False,
            "at": reading.at if reading else None,
            "when": engine.clock(reading.at) if reading else "",
            "age": engine.humanise(time.time() - reading.at) if reading else "",
            "foot": self.footer(reading, state),
            "tip": self.tooltip(reading, state),
            "window_spend": engine.money(self.store.spent_within(30) or 0.0, currency) if reading else "",
        }
        return body
    ...
    def update_tray(self, force: bool = False) -> None:
        ...
        try:
            state = self.payload()
            self.icon.title = "DeepSeek balance " + (
                str(state["amount"]) if state["balance"] is not None else (state["problem"] or "no reading")
            )
            if force or getattr(self, "_tray_state", None) != state["state"]:
                self._tray_state = state["state"]
                self.icon.icon = tray_image(state["state"])
    ...
    def start_tray(self) -> None:
        menu = pystray.Menu(
            pystray.MenuItem("Refresh balance", self.refresh_from_tray, default=True),
    ...
    def refresh_from_tray(self, *_args) -> None:
        self.refresh_balance("the tray")
    ...
    def stay_on_desktop(self) -> None:
        ...
        if self._on_desktop_layer:
            ...
            log(
                "desktop hidden again — card back on top"
                if on_top
    ...
    def after_start(self) -> None:
        ...
        log(f"window ready (key={'yes' if engine.read_key() else 'no'}, readings={len(self.store.rows)})")
        self.start_tray()
...
def dump_preview() -> int:
    """Write the browser preview payload (ui/state.preview.json) from a live read."""
    widget = Widget()
    widget.refresh_balance("preview", wait=True)
    state = widget.payload()
    state["prefs"] = {**state["prefs"], "on_top": True, "notify": True, "glass": True}
    PREVIEW_FILE.write_text(json.dumps(state, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(f"wrote {PREVIEW_FILE}")
    print(f"  state : {state['state']}  {state['amount']}")
    print(f"  foot  : {state['foot']}")
    print(f"  reads : {state['reads']}")
    return 0


def self_check() -> int:
    """Environment, the key (masked) and one live balance line."""
    ...
    key = engine.read_key()
    print(f"key file    : {engine.DEFAULT_KEY_FILE}")
    print(f"key         : {'MISSING or empty' if not key else key[:8] + '****' + key[-4:] + f'  ({len(key)} chars)'}")
    print(f"history     : {len(Widget().store.rows)} readings in {ROOT / 'history.json'}")
    try:
        reading = engine.fetch(key or "")
        print(
            f"balance     : {engine.money(reading.total, reading.currency)}"
            f"   granted {engine.money(reading.granted, reading.currency)}"
            f"   topped up {engine.money(reading.topped_up, reading.currency)}"
        )
        print(f"available   : {reading.is_available}   currency: {reading.currency}")
    except engine.BalanceError as error:
        print(f"balance     : unavailable [{error.kind}] {error}")
    return 0
...
def run(debug: bool, ui_test: bool = False) -> int:
    ...
    log(
        ...
        f"key={'yes' if engine.read_key() else 'no'}, glass={widget.config.get('glass', True)})"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.state:
        widget = Widget()
        widget.refresh_balance("--state", wait=True)
        payload = widget.payload()
        print(json.dumps(payload, indent=2, ensure_ascii=False))
        return 0 if payload["state"] not in ("error", "no_key", "auth") else 1
